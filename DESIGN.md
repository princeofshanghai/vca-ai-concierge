  ...
  patterns:
    ...
    message-enter:
      ...
      opacity:
        from: 0
        to: 1
      intent: "Use when user, assistant, or representative messages appear in the thread."
    assistant-thinking:
...
- **Background neutral soft (`#F4F2EE`)** is used for representative chat messages.
...
Every icon-only chat header action uses the shared styled tooltip below the control on pointer hover and keyboard focus. Render action tooltips in a shared overlay layer so chat-container and component-preview clipping never crops them, align edge tooltips inward within the viewport, and keep tooltip copy synchronized with the accessible button label. Header tooltips do not replace accessible labels or introduce long-press behavior on touch devices.

When a live sales consultant has joined the conversation, the header identity changes from the AI mark and `Contact sales` title to the representative identity. `Sales consultant` is the canonical customer-facing role name for LTS Hiring; `representative`, `live agent`, `AE`, and `SDR` remain internal system or component terms. Use a 32px circular entity with a small active presence badge over the lower-right edge, an 8px gap, and a single-line `heading-md` representative name; do not show the role in the chat shell header. The same representative identity should appear in the minimized tray when the tray shell is docked. Representative message metadata uses a 24px circular entity with the same small active presence badge treatment. Shell control behavior should stay consistent within a selected review shell as the conversation moves between default, docked, expanded, and handoff states.
...
Side panels are focused task or detail surfaces inside the chat container, not independent product surfaces. Preserve the chat header and conversation context so users can complete a task or inspect related content without leaving the conversation. Component-library demos should show the full chat-container context by default; a panel-only view is an inspection convenience, not a supported user-facing state. On mobile, the side panel may replace the visible history within the same container and use Back to chat to return.

High-value scheduling may add a side panel in either collapsed or expanded chat states: the left panel preserves the chat context while the right panel carries the scheduling task on the light neutral background token. The chat header remains visible above both panels. The scheduling panel starts with a `Back to chat` affordance that closes the side panel and preserves the current chat state. When opened from collapsed chat, the wider two-panel surface stays right-aligned; only expanded chat centers the wider surface. The collapsed scheduling surface should be about 896px wide with a 3:5 chat-to-scheduler split, yielding about 336px of chat context at max width. The expanded scheduling surface should be about 1280px wide with the same 3:5 split, yielding about 480px of chat context at max width. Use the named schedule layout rule for these panes rather than ad hoc arbitrary grid strings. While the scheduler is open, the in-thread sales consultant card is passive context, not a second CTA. After booking, the side panel closes and the sales consultant card is replaced by a booked confirmation card with no manage-booking action.

### Messages
Assistant messages are plain text on the panel surface. User messages use the pale blue message surface. Representative messages use the warm neutral message surface.
...
In scheduled sales consultant flows, the first recommendation card uses one CTA to start booking. After click, the same card transitions through a matching state, then a matched sales consultant state with the sales consultant entity and a single scheduling CTA. These match cards use a white surface with the AI border token across initial, matching, matched, scheduling, and booked states, so they read as lightweight system cards rather than blue message bubbles. The booked-card heading uses heading-md; the date uses body-sm; the contact delivery line uses body-sm with text-meta.
...
Each prompt follows the width of its visible label, stays aligned to the start of its prompt group, and wraps long labels within the available chat width. The visible label may be shorter than the prompt that is sent, but the component should keep the full prompt available to assistive tech and hover affordances.

Prompt groups are layout-only and must remain inside the response attachment that owns their spacing and reveal timing. Stack prompts vertically in the standard-width panel. In the expanded panel, arrange prompts inline and wrap them when needed. Changing the panel variant should reflow the existing prompt controls without remounting them or changing the thinking, streaming, attachment-reveal, or response-runway behavior.
